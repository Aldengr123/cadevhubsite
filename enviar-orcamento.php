<?php
/* ============================================================
   CADev Hub — Recebimento de orçamentos (com modo DEBUG)
...
$rows .= row('Negócio', $d['negocio'] ?? '');
$rows .= row('Instagram / site', $d['link'] ?? '');
$rows .= row('Presença digital', $d['presenca'] ?? '');
$rows .= row('Tipo de projeto', $d['tipo'] ?? '');
$rows .= row('Objetivo', $d['objetivo'] ?? '');
$rows .= row('Faturamento', $d['faturamento'] ?? '');
$rows .= row('Orçamento previsto', $d['orcamento'] ?? '');
$rows .= row('Decisor', $d['decisor'] ?? '');
$rows .= row('Prazo', $d['prazo'] ?? '');
...
$html = '<!doctype html><html><body style="margin:0;background:#0C0E0E;padding:24px">'
  ...
  . '<div style="font:400 13px Arial;color:#ACB9B9;margin-top:4px">Nova solicitação de orçamento</div></td></tr>'
...
try {
  ...
  error_log('[CADev orcamento] ' . $ex->getMessage());
